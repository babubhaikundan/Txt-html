...
def extract_topic(title):
    """
    Extract base topic by removing #number patterns
    Always returns base topic for proper grouping
    """
    # Remove patterns like #1, #2, #8, etc. at the end
    base_topic = re.sub(r'\s*#\d+\s*
...
, '', title).strip()
    return base_topic  # Always return, no None check
